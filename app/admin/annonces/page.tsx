      ...
      <SlidePanel open={panel === 'form'} onClose={() => setPanel(null)} title={selected ? "Modifier l'annonce" : 'Nouvelle annonce'} width="500px">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', paddingTop: '4px' }}>
            <button onClick={saveAnnonce} disabled={saving} style={{ flex: 1, padding: '11px', backgroundColor: D.yellow, border: 'none', borderRadius: D.radiusSm, color: '#000', fontSize: '13px', fontWeight: '700', cursor: 'pointer', opacity: saving ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              {saving ? <><YelenLoader size={12} color="#000"/>Sauvegarde…</> : selected ? 'Enregistrer' : "Créer l'annonce"}
            </button>
