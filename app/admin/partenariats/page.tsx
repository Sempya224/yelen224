...
// File de modération des demandes de partenariat Yelen (chantier
// 26/07/2026). Gabarit calqué sur app/admin/feedback/page.tsx.
...
type Demande = {
  ...
  impact_communaute: string; categorie: string; site_web: string; contact_nom: string;
  contact_email: string; contact_telephone: string | null; infos_complementaires: string | null;
...
export default function PartenariatsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await fetch(`/api/admin/partenariats?statut=${statut}`)
  ...
  async function valider(id: string) {
    setBusyId(id)
    const res = await fetch(`/api/admin/partenariats/${id}/valider`, { method: 'POST' })
    setBusyId(null)
    if (res.ok) load()
  }
  ...
  async function refuser(id: string) {
    ...
    setBusyId(id)
    const res = await fetch(`/api/admin/partenariats/${id}/refuser`, {
  ...
  return (
    <div>
      <h1 style={{ color: D.text, fontSize: '22px', fontWeight: '800', marginBottom: '4px' }}>Demandes de partenariat</h1>
      <p style={{ color: D.textSub, fontSize: '13px', marginBottom: '20px' }}>
        Candidatures des institutions au programme de partenariat Yelen.
      </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {items.map(d => (
            <div key={d.id} style={{ backgroundColor: D.surface, border: `1px solid ${D.border}`, borderRadius: '14px', padding: '16px' }}>
              ...
              <p style={{ color: D.textSub, fontSize: '13px', lineHeight: 1.6, margin: '0 0 8px' }}><strong style={{ color: D.text }}>Impact communauté : </strong>{d.impact_communaute}</p>
