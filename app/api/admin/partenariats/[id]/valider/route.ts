...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await supabaseAdmin.from('institutions').update({ partenaire_statut: 'approuve' }).eq('id', demande.institution_id)

    await supabaseAdmin.from('notifications').insert({
      destinataire_id: demande.institution_id,
      destinataire_type: 'institution',
      titre: 'Partenariat Yelen approuvé',
      message: "Votre demande de partenariat a été approuvée. Vous pouvez maintenant créer vos offres depuis l'onglet Mes offres.",
      type: 'partenariat',
