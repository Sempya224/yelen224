...
export async function GET(request: NextRequest) {
  try {
    ...
    if (download) {
      const { data: doc } = await supabaseAdmin.from("citoyen_documents").select("url").eq("id", download).eq("citoyen_id", user.id).maybeSingle();
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select("id,institution_id,rdv_id,sens,type,label,description,statut,taille,type_mime,created_at,traite_le,institutions(name)")
