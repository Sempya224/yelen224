    ...
    <div style={{ padding: "16px", paddingBottom: "100px", animation: "fadeUp 0.2s ease" }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ backgroundColor: C.bgCard, borderRadius: "16px", padding: "40px 20px", textAlign: "center", border: `1px solid ${C.border}` }}>
          <p style={{ color: C.t2, fontSize: "13px" }}>{clients.length === 0 ? "Aucun client pour l'instant — vos premiers RDV apparaîtront ici." : "Aucun client trouvé."}</p>
        </div>
      ) : (
        <div style={{ backgroundColor: C.bgCard, border: `1px solid ${C.border}`, borderRadius: "16px", overflow: "hidden" }}>
          {filtered.map(c => {
      ...
      {selected && (
        <div className="client-fiche-overlay" style={{ position: "fixed", inset: 0, zIndex: 1000, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center", animation: "fadeIn 0.2s ease" }} onClick={() => setSelected(null)}>
          ...
          <div onClick={e => e.stopPropagation()} className="client-fiche-panel" style={{ position: "relative", backgroundColor: C.bgCard, borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: "560px", maxHeight: "88svh", overflowY: "auto", border: `1px solid ${C.border2}`, borderBottom: "none", animation: "slideUp 0.3s ease" }}>
              ...
              <div>
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{ backgroundColor: C.bg3, borderRadius: "12px", padding: "4px 10px", maxHeight: "200px", overflowY: "auto", marginBottom: "8px" }}>
                    {notesLoading ? (
                      <div style={{ padding: "14px", color: C.t3, fontSize: "11.5px", textAlign: "center" }}>Chargement…</div>
                    ) : (() => {
                  ...
                  {!readOnly && (
                    <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      />
                      <button onClick={ajouterNote} disabled={savingNote || !newNoteText.trim()} className="tap" style={{ backgroundColor: C.purple, color: "#fff", border: "none", borderRadius: "10px", padding: "0 16px", fontSize: "12px", fontWeight: "800", cursor: "pointer", opacity: savingNote || !newNoteText.trim() ? 0.5 : 1 }}>
                        {savingNote ? "…" : "Ajouter"}
                      </button>
                    </div>
                  )}
                ...
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{ backgroundColor: C.bg3, borderRadius: "12px", padding: "10px", maxHeight: "260px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "8px" }}>
                    {messagesLoading ? (
                      <div style={{ padding: "10px", color: C.t3, fontSize: "11.5px", textAlign: "center" }}>Chargement…</div>
                    ) : messages.length === 0 ? (
                      <EmptyState
            ...
            {!readOnly && (
              <button onClick={envoyerRappel} disabled={sendingRappel} className="tap" style={{ width: "100%", backgroundColor: C.bgCard, color: C.gold, fontSize: "13px", fontWeight: "800", padding: "13px", borderRadius: "12px", border: `1px solid ${C.gold}40`, cursor: "pointer", opacity: sendingRappel ? 0.6 : 1 }}>
                {sendingRappel ? "…" : "Envoyer un rappel"}
              </button>
            )}
