...
function FieldCard({ C, title, hint, placeholder, field, savedValue, savedDate, onToast, onSaved }: {
        ...
        {editing ? (
          <>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "12px" }}>
              ...
              <button onClick={save} disabled={saving} className="tap" style={{ backgroundColor: saving ? C.bg3 : C.gold, color: saving ? C.t3 : "#000", border: "none", borderRadius: "10px", padding: "9px 18px", fontSize: "12.5px", fontWeight: "800", cursor: saving ? "not-allowed" : "pointer" }}>
...
export function ConditionsInformationsTab({ instId, onToast }: { instId: string; onToast: (msg: string, color?: string) => void }) {
  ...
  if (loading) {
    return <div style={{ padding: "48px", textAlign: "center", color: C.t3, fontSize: "13px" }}>Chargement…</div>;
  }
