      ...
      <div style={{ maxWidth: "720px", margin: "0 auto" }}>
        ...
        <div style={{ backgroundColor: C.bgCard, border: `1px solid ${C.border}`, borderRadius: "16px", padding: "16px", display: "flex", flexDirection: "column", gap: "14px", marginBottom: "18px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              <label style={fieldLabel(C)}>Prénom *</label>
              <input value={form.prenom} onChange={e => fc("prenom", e.target.value)} disabled={readOnly} style={fieldInput(C)}/>
            </div>
            <div>
              <label style={fieldLabel(C)}>Nom *</label>
              <input value={form.nom} onChange={e => fc("nom", e.target.value)} disabled={readOnly} style={fieldInput(C)}/>
            </div>
          </div>
          <div>
            <label style={fieldLabel(C)}>Rôle dans l'institution</label>
            <input value={form.role} onChange={e => fc("role", e.target.value)} placeholder="Ex: Directeur, Gérant, Responsable accueil…" disabled={readOnly} style={fieldInput(C)}/>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              <label style={fieldLabel(C)}>Téléphone personnel</label>
              <input value={form.phone} onChange={e => fc("phone", e.target.value)} disabled={readOnly} style={fieldInput(C)}/>
            </div>
            <div>
              <label style={fieldLabel(C)}>Email personnel</label>
              <input type="email" value={form.email} onChange={e => fc("email", e.target.value)} disabled={readOnly} style={fieldInput(C)}/>
            </div>
          </div>
        </div>
