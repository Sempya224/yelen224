...
const Illu: Record<Suivi["type"], (size?: number) => React.ReactNode> = {
  rdv: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#FEF3C7"/><rect x="14" y="14" width="24" height="22" rx="3" fill="#F5A623"/><rect x="19" y="10" width="3" height="7" rx="1.5" fill="#F5A623"/><rect x="30" y="10" width="3" height="7" rx="1.5" fill="#F5A623"/><rect x="18" y="24" width="5" height="5" fill="#fff"/><rect x="29" y="24" width="5" height="5" fill="#fff"/></svg>),
  avis: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#FEF3C7"/><polygon points="26,12 30.5,21.5 41,23 33.5,30 35.5,40.5 26,35.5 16.5,40.5 18.5,30 11,23 21.5,21.5" fill="#F5A623"/></svg>),
  documents: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#DBEAFE"/><path d="M17 12h13l7 7v21H17z" fill="#2563EB"/><path d="M30 12v7h7" fill="#93C5FD"/><rect x="21" y="26" width="10" height="2.4" rx="1.2" fill="#DBEAFE"/><rect x="21" y="31" width="10" height="2.4" rx="1.2" fill="#DBEAFE"/></svg>),
  demarche_retard: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#FEE2E2"/><circle cx="26" cy="26" r="14" fill="none" stroke="#E11D48" strokeWidth="3"/><line x1="26" y1="26" x2="26" y2="18" stroke="#E11D48" strokeWidth="3" strokeLinecap="round"/><line x1="26" y1="26" x2="32" y2="30" stroke="#E11D48" strokeWidth="3" strokeLinecap="round"/></svg>),
  demarche_echeance: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#FFEDD5"/><path d="M18 14h16v6c0 4-3 5-3 6s3 2 3 6v6H18v-6c0-4 3-5 3-6s-3-2-3-6z" fill="#EA580C"/></svg>),
  depense: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#EDE9FE"/><rect x="14" y="30" width="7" height="10" rx="1.5" fill="#C4B5FD"/><rect x="23" y="24" width="7" height="16" rx="1.5" fill="#A78BFA"/><rect x="32" y="16" width="7" height="24" rx="1.5" fill="#7C3AED"/></svg>),
  interet: (s = 52) => (<svg viewBox="0 0 52 52" width={s} height={s}><circle cx="26" cy="26" r="26" fill="#CFFAFE"/><circle cx="26" cy="26" r="13" fill="none" stroke="#0891B2" strokeWidth="3"/><circle cx="26" cy="26" r="7" fill="none" stroke="#0891B2" strokeWidth="3"/><circle cx="26" cy="26" r="2.4" fill="#0891B2"/></svg>),
};
...
export function SuivisSection({ isDark, onCreerDemarche }: { isDark: boolean; onCreerDemarche: (titre: string, categorie: "personnel" | "professionnel") => void }) {
  ...
  const ombre = isDark ? "none" : "0 1px 4px rgba(0,0,0,0.04)";
    ...
    <div style={{ marginBottom: "24px" }}>
      ...
      {suivis.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {suivis.map((s, i) => (
            <button key={s.id} onClick={() => agir(s)} className="tap" style={{ display: "flex", alignItems: "center", gap: "12px", textAlign: "left", padding: "11px 12px", borderRadius: "16px", border: `1px solid ${brd}`, backgroundColor: card, boxShadow: ombre, cursor: "pointer", animation: `suiviIn 0.3s ease ${i * 0.06}s both` }}>
              <div style={{ flexShrink: 0, width: "44px", height: "44px" }}>{Illu[s.type](44)}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: t1, fontSize: "13px", fontWeight: "800", marginBottom: "1px" }}>{s.titre}</div>
                <div style={{ color: t2, fontSize: "11.5px", lineHeight: "1.4" }}>{s.description}</div>
              </div>
              <div style={{ flexShrink: 0, width: "26px", height: "26px", borderRadius: "50%", backgroundColor: card2, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={t2} strokeWidth="2.5" strokeLinecap="round"><path d="m9 18 6-6-6-6"/></svg>
              </div>
            </button>
          ))}
        </div>
