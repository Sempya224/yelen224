...
// 20260720000002, texte libre, inchangée). Dropdown de suggestions :
// chantier Taxonomie des activités (Phase 3.5, 20/08/2026) — devient la
// liste des activités réelles de la catégorie de l'institution
// (activite_categorie_id/activites) au lieu de la liste plate des 9
// SECTEURS. Repli sur l'ancienne liste SECTEURS si l'institution n'a pas
// encore de catégorie renseignée (comptes non migrés, jamais un dropdown
// vide qui bloquerait la création de service). Statut = mappé sur le
// booléen réel is_active (Actif/Suspendu), pas un enum à 4 valeurs qui
...
const FALLBACK_CATEGORIE_OPTIONS = SECTEURS.map(s => s.label);

const SECTEUR_COLOR: Record<string, CategoryColorKey> = {
  "Santé": "red", "Administratif": "blue", "Financier": "green", "Juridique": "purple",
  "Beauté / Bien-être": "teal", "Artisanat": "orange", "Services divers": "teal",
};
...
function ServiceForm({ onSave, onCancel, saving, initial, categorieOptions }: {
  ...
  const initialKnown = !!(initial?.categorie && categorieOptions.includes(initial.categorie) && initial.categorie !== CATEGORIE_AUTRE);
  ...
  async function submit() {
    ...
    if (!prix || isNaN(+prix) || +prix <= 0)    { setErr(`Entrez un prix valide en ${DEVISE_LABEL} (ex: 5000)`); return; }
    ...
    <div>
      ...
      <div className="svc-form-fields">
        <div className="svc-form-span2">
          <FormField C={C} label="Nom du service" required placeholder="Ex: Consultation médicale, Coupe homme, Massage…" value={nom} onChange={setNom} name="nom"/>
        </div>
...
function ServiceFormSheet({ target, onSave, onCancel, saving, categorieOptions }: {
  ...
  return (
    <div className="svc-form-overlay" style={{ position: "fixed", inset: 0, zIndex: 1000, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center", animation: "fadeIn 0.2s ease" }} onClick={onCancel}>
      ...
      <div onClick={e => e.stopPropagation()} className="svc-form-panel" style={{ position: "relative", backgroundColor: C.bgCard, borderRadius: "24px 24px 0 0", padding: "24px 20px 32px", width: "100%", maxWidth: "560px", maxHeight: "90svh", overflowY: "auto", border: `1px solid ${C.border2}`, borderBottom: "none", animation: "slideUp 0.3s ease" }}>
        ...
        <ServiceForm onSave={onSave} onCancel={onCancel} saving={saving} initial={target === "new" ? null : target} categorieOptions={categorieOptions}/>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-start", gap: "12px", marginBottom: "20px" }}>
        ...
        {saving && <div style={{ flexShrink: 0, marginTop: "3px" }}><YelenLoader size={14} color={C.blue}/></div>}
      </div>

      {loading ? (
        <div style={{ display: "flex" }}><YelenLoader size={20} label="Chargement…" labelColor={C.t3}/></div>
      ) : loadError ? (
...
export function ServicesTab({ instId }: { instId: string }) {
  ...
  // Chantier Taxonomie des activités (Phase 3.5, 20/08/2026) — activités
  // réelles de la catégorie de l'institution, pour peupler le dropdown
  // catégorie du formulaire service payant. null tant que non résolu ou si
  // l'institution n'a pas encore de activite_categorie_id (repli sur
  // FALLBACK_CATEGORIE_OPTIONS, jamais un dropdown vide).
  const [activiteLabels, setActiviteLabels] = useState<string[] | null>(null);
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/api/institution/profile?institution_id=${instId}`);
      const j = res.ok ? await res.json().catch(() => null) : null;
      const categorieId = j?.institution?.activite_categorie_id;
      if (!categorieId) { setActiviteLabels(null); return; }
      const { data } = await supabase.from("activites").select("label").eq("categorie_id", categorieId).eq("statut", "active").order("ordre");
      setActiviteLabels(data && data.length > 0 ? data.map(a => a.label) : null);
    })();
  }, [instId]);

  const categorieOptions = [...(activiteLabels ?? FALLBACK_CATEGORIE_OPTIONS), CATEGORIE_AUTRE];
  ...
  return (
    <div style={{ padding: "16px", animation: "fadeUp 0.2s ease" }}>
      ...
      {formTarget && <ServiceFormSheet target={formTarget} onSave={handleSaveForm} onCancel={() => setFormTarget(null)} saving={saving} categorieOptions={categorieOptions}/>}
