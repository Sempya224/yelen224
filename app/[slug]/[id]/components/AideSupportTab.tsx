...
// Écran dédié "Support" — extrait du bloc "Support & Légal" de page.tsx
// (chantier éclatement de Paramètres, 14/09/2026). Purement des liens
// externes/mailto, aucune donnée à charger.
import { useTheme } from "@/components/ThemeProvider";
import { T, type ThemeTokens, toCardTokens } from "../theme";
import { Card } from "@/components/ui/Card";

type SupportLink = { label: string; href: string; color: string };

export function AideSupportTab() {
  const { theme } = useTheme();
  const C = T[theme] as ThemeTokens;

  const links: SupportLink[] = [
    { label: "Aide et FAQ",          href: "/guide-prestataire",          color: C.blue },
  ...
  return (
    <div style={{ padding: "16px", paddingBottom: "100px", animation: "fadeUp 0.2s ease" }}>
      <div style={{ maxWidth: "720px", margin: "0 auto" }}>
        <h1 style={{ color: C.t1, fontSize: "22px", fontWeight: "800", letterSpacing: "-0.5px", marginBottom: "16px" }}>Support</h1>
