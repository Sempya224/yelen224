...
// réservé à equipe.read_full). PIN/passkeys/appareils institution-wide
// restent exclusifs à "Sécurité du compte" (parametres-securite, admin
// uniquement, un lien y renvoie) — mais la 2FA TOTP est un réglage
// personnel réel de CE membre (institution_membres.totp_enabled), exposée
// ici directement pour les 4 rôles qui n'ont pas accès à cet onglet-là
// (TotpSection.tsx, partagée avec SecuriteCompteTab.tsx).
...
type JournalEntree = { id: string; action: string; created_at: string };
  ...
  return (
    <div style={{ padding: "16px", paddingBottom: "100px", animation: "fadeUp 0.2s ease" }}>
      <div style={{ maxWidth: "480px", margin: "0 auto" }}>
        ...
        <p style={{ color: C.t2, fontSize: "13px", marginBottom: "18px" }}>Votre identité administrative, votre méthode d&apos;accès et l&apos;activité de ce compte.</p>
        ...
        <Card tokens={toCardTokens(C)} padding="18px" style={{ marginBottom: "14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "12px" }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: C.t3, fontSize: "12px", marginTop: "2px", display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                ...
                {moi.compte_principal && <span style={{ backgroundColor: `${C.gold}15`, color: C.gold, fontSize: "9px", fontWeight: "800", padding: "2px 6px", borderRadius: "8px" }}>Compte principal</span>}
          ...
          <div style={{ color: C.t3, fontSize: "11.5px", lineHeight: 1.5 }}>{ROLE_DESCRIPTIONS[moi.role]}</div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: "12px", paddingTop: "12px", borderTop: `1px solid ${C.border}`, fontSize: "11px" }}>
            <div>
        ...
        {showChangerPin ? (
          ...
          <Card tokens={toCardTokens(C)} padding="16px" style={{ marginBottom: "14px", display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        {/* ── SÉCURITÉ ──
            Admin : PIN/2FA/passkeys/appareils institution-wide restent
            exclusifs à "Sécurité du compte" (un seul lien, pas de double
            point d'entrée pour la même personne). Les 4 autres rôles n'y
            ont pas accès (TAB_MATRIX) mais la 2FA TOTP et les passkeys
            SONT des réglages PERSONNELS réels de CE membre
            (institution_membres.totp_enabled,
        ...
        {moi.role === "admin" ? (
          ...
          <>
            <SectionTitle C={C} label="Sécurité"/>
            <Card tokens={toCardTokens(C)} padding="16px" style={{ marginBottom: "14px" }}>
              ...
              {activite.map(e => (
                <div key={e.id} style={{ display: "flex", alignItems: "center", gap: "10px", backgroundColor: C.bg3, borderRadius: "10px", padding: "9px 11px" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: C.t3, fontSize: "10px", marginTop: "1px" }}>{timeAgo(e.created_at)}</div>
                  </div>
                </div>
              ))}
            </div>
